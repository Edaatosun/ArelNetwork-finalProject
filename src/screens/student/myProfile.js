...
export default function Profile() {
    ...
    const InfoItem = ({ label, value }) => (
        <View className="flex-row justify-between items-center bg-[#b6dcfa] p-3 rounded-xl mb-2">
            <Text
                className="text-[#0f172a] font-semibold  mr-2"
                numberOfLines={1}
            >
                {value}
            </Text>
    ...
    return (
        <ScrollView className="flex-1 bg-gray-100">
            ...
            {/* Profil bilgileri ve resim alanı */}
            <View className="absolute top-56 w-full items-center z-20">
                <View className="w-full bg-white h-screen rounded-2xl shadow-md p-2 items-center">
                    ...
                    {/* Kişisel Bilgi Kartı ve CV Butonları için ayrı bir View */}
                    <View className="px-5 w-full"> {/* Profil kartının altına gelecek şekilde ayarlandı ve yatayda hizalama için px-5 eklendi */}
                        ...
                        <View className="mt-5 space-y-4 mb-10">
                            {cvUrl ? (
                                <View className="flex-row justify-between ">
                                    <TouchableOpacity
                                        ...
                                        className="flex-row items-center bg-blue-600 p-4 rounded-2xl shadow-md mr-10 " // mr-10 ile sağa daha fazla boşluk ekledik
                                    >
                                    ...
                                    <TouchableOpacity
                                        onPress={deleteCV}
                                        className="flex-row items-center bg-red-600 p-4 rounded-2xl shadow-md mr-10" // mr-10 ile sağa daha fazla boşluk ekledik
                                    >
                            ...
                            {userInfo.photo ? (
                                <View className="flex-row items-center bg-red-600 p-4 rounded-2xl shadow-md justify-between">
                                    <TouchableOpacity
                                        onPress={deletePhoto}

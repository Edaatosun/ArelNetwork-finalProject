...
export default function DetailsJob() {
    ...
    const { item_id } = route.params; // Bu, jobId olacak
    const fadeAnim = useRef(new Animated.Value(0)).current;
    const [job, setJob] = useState(null);
    const [loading, setLoading] = useState(true);
    const [isApplied, setIsApplied] = useState(false);
    const [resumeData, setResumeData] = useState(null); // resume URL'si burada saklanacak
    ...
    const handleApply = async () => {
        ...
        try {
            ...
            formData.append('resume', { // Backend'de req.file olarak alınacak
                uri: uri,
                name: name,
        ...
        <View className="flex-1 bg-white">
            ...
            <Animated.View style={{ opacity: fadeAnim }} className="flex-1">
                <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
                    ...
                    <View className="p-4">
                        <Text className="text-2xl font-bold mb-4 text-black ">{job.jobTitle}</Text>

                        <View className="flex-row items-center mb-2">
                            <FontAwesome5 name="home" size={20} color="#4B5563" />
                            <Text className="ml-2 text-gray-700 text-base">{job.company}</Text>
                        </View>

                        <View className="flex-row items-center mb-2">
                            <Icon name="location-outline" size={20} color="#4B5563" />
                            <Text className="ml-2 text-gray-700 text-base">{job.location}</Text>
                        </View>

                        <View className="flex-row items-center mb-2">
                            <Icon name="calendar-outline" size={20} color="#4B5563" />
                            <Text className="ml-2 text-gray-700 text-base">
                                {job.fromDate ? new Date(job.fromDate).toLocaleDateString('tr-TR') : 'Tarih Yok'} - {job.toDate ? new Date(job.toDate).toLocaleDateString('tr-TR') : 'Tarih Yok'}
                            </Text>
                        </View>
                        ...
                        {job.jobField && job.jobField.length > 0 && (
                            <View className="flex-row items-center mb-4">
                                ...
                                <Text className="ml-2 text-gray-700 text-base">
                                    {Array.isArray(job.jobField) ? job.jobField.join(', ') : job.jobField}
                                </Text>
                            </View>
                        )}
                    </View>
